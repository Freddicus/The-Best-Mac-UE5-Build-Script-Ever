...
# Update CURRENT_PROJECT_VERSION and MARKETING_VERSION in the UE-generated xcconfig so
# that Info.plist is stamped correctly on the next Xcode archive.
# The xcconfig lives at: Intermediate/ProjectFiles/XcconfigsMac/<LONG_NAME>.xcconfig
# It is generated by UE's GenerateProjectFiles — this function only rewrites the two
# version keys; everything else is left untouched.
# Only runs when VERSION_MODE != NONE.
update_xcconfig_versions() {
  ...
  if [[ ! -f "$_xcconfig_path" ]]; then
    info "xcconfig not found at $_xcconfig_path — skipping Info.plist version stamp (run GenerateProjectFiles first)"
    return 0
  fi
  ...
  local _tmp _line _found_cpv=0 _found_mv=0
  _tmp="$(/usr/bin/mktemp "${TMPDIR:-/tmp}xcconfig_update_XXXXXX")"

  while IFS= read -r _line || [[ -n "$_line" ]]; do
    if [[ "$_line" == CURRENT_PROJECT_VERSION* ]]; then
