...
| `ENABLE_GAME_CENTER` | `1` = add `com.apple.developer.game-center` entitlement for Mac and iOS. Mac: seeds `Build/Mac/Resources/<Project>.entitlements` and writes **both** `PremadeMacEntitlements` and `ShippingSpecificMacEntitlements` to `DefaultEngine.ini` (Shipping configs prefer the latter — see [gotchas](gotchas.md#game-center-entitlements-must-be-committed-source-files--intermediate-only-paths-dont-reach-signing)) so `GenerateProjectFiles` bakes `CODE_SIGN_ENTITLEMENTS` into every config's xcconfig. Mac archive/export also use `-allowProvisioningUpdates` so `xcodebuild` can auto-create the Developer ID Direct Distribution profile that authorizes the entitlement at AMFI launch time. iOS: seeds `Build/IOS/Resources/<Project>.entitlements` and passes `CODE_SIGN_ENTITLEMENTS=<path>` directly to `xcodebuild archive` (UBT has no `PremadeMacEntitlements` equivalent for iOS — the build setting override is the reliable path). Also writes `bEnableGameCenterSupport=True` for the UAT cook step. `0` = remove the key and undo ini changes. Unset = no-op. **Commit both seeded `.entitlements` files** so they survive project regeneration. Your Apple team must have Game Center enabled on the bundle ID in App Store Connect, otherwise the auto-provisioning step has nothing to mint. |
