...
| `MAC_RHI_CHECK` | `1` (default) = pre-flight resolves the project's Mac `TargetedRHIs` and prompts before shipping a build that omits `SF_METAL_SM5`, which fails to launch on every M1 Mac and on any Apple Silicon Mac running macOS 14 or older. `0` = skip the prompt. The resolved-targeting report always prints regardless — only the prompt is suppressed. Answering `x` at the prompt persists `MAC_RHI_CHECK=0` to `.env`. Non-TTY runs print the warning and continue without prompting. See [gotchas](gotchas.md#your-mac-cannot-run-the-shader-path-your-players-will). |
...
./ship.sh --no-rhi-check                   # skip the Mac TargetedRHIs pre-flight prompt
