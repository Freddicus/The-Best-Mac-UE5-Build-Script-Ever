...
| `Build/Mac/Resources/<Project>.entitlements` | only when `ENABLE_GAME_CENTER=1` | Registers `com.apple.developer.game-center` via `PremadeMacEntitlements` in `DefaultEngine.ini`; `GenerateProjectFiles` then bakes `CODE_SIGN_ENTITLEMENTS` into the xcconfig. macOS has no UBT ini boolean equivalent to iOS's `bEnableGameCenterSupport` — the file must exist. See [gotchas](gotchas.md#macos-game-center-has-no-ubt-ini-boolean--the-entitlements-file-must-exist). | yes | disable `ENABLE_GAME_CENTER` |
